...
import { generateLabCardArt, generateLabHeroArt } from '@/lib/lab-art'
...
interface LabListContentProps {
  experiments: LabExperiment[]
}

// ─── Component ──────────────────────────────────────────────────────────────────

export default function LabListContent({ experiments }: LabListContentProps) {
  ...
  const gridExperiments =
    activeTag === 'All'
      ? experiments.slice(1)
      : experiments.filter((e) => e.tags.includes(activeTag) && e.slug !== featured.slug)

  // Build tag counts
  const tagCounts = new Map<string, number>()
  experiments.forEach((e) => e.tags.forEach((t) => tagCounts.set(t, (tagCounts.get(t) || 0) + 1)))
  const tags = [
    { name: 'All', count: experiments.length },
    ...Array.from(tagCounts.entries()).map(([name, count]) => ({ name, count })),
  ]

  // GSAP entrance animations — same as BlogListContent
  useGSAP(
    () => {
  ...
  return (
    <div ref={pageRef} className="relative min-h-screen bg-[#f8f8f6]">
      ...
      <main>
        ...
        <Link href={`/lab/${featured.slug}`}>
          <section className="flex flex-col md:flex-row group cursor-pointer overflow-hidden pt-24">
            ...
            <div
              ...
              style={{ background: generateLabHeroArt(featured.slug, featured.tags), opacity: 0 }}
            >
              <LazyIframe src={featured.src} title={featured.title} />
              <div
        ...
        <section ref={gridRef} className="px-8 md:px-14 lg:px-16 py-8 md:py-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
            {gridExperiments.map((exp) => (
              <Link key={exp.slug} href={`/lab/${exp.slug}`}>
                <article
                  ...
                  style={{
                    background: generateLabCardArt(exp.slug, exp.tags),
                    minHeight: '380px',
                  }}
                >
                  <div
      ...
      <LabFooter heroArt={generateLabHeroArt(featured.slug, featured.tags)} />
    </div>
  )
}

// ─── Lazy Iframe (only mounts when visible, unmounts when offscreen) ────────────
...
function LazyIframe({ src, title }: { src: string; title: string }) {
  ...
  return (
    <div ref={ref} className="absolute inset-0 pointer-events-none" style={{ opacity: 0.4 }}>
      {visible && (
        <iframe
          src={src}
...
function LabFooter({ heroArt }: { heroArt: string }) {
  ...
  useGSAP(
    () => {
      const paths = logoRef.current?.querySelectorAll('.lab-footer-logo-path')
